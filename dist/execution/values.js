  ...
  var _loop = function _loop(i) {
    ...
    if (!(0, _definition.isInputType)(varType)) {
      ...
      if (!hasValue && varDefNode.defaultValue) {
        ...
        errors.push(new _error.GraphQLError(hasValue ? "Variable \"$".concat(varName, "\" of non-null type ") + "\"".concat(String(varType), "\" must not be null.") : "Variable \"$".concat(varName, "\" of required type ") + "\"".concat(String(varType), "\" was not provided."), [varDefNode]));
      } else if (hasValue) {
        if (value === null) {
...
function getArgumentValues(def, node, variableValues) {
  ...
  for (var i = 0; i < argDefs.length; i++) {
    ...
    if (!hasValue && argDef.defaultValue !== undefined) {
      ...
      if (isNull) {
        throw new _error.GraphQLError("Argument \"".concat(name, "\" of non-null type \"").concat(String(argType), "\" ") + 'must not be null.', [argumentNode.value]);
      } else if (argumentNode && argumentNode.value.kind === _kinds.Kind.VARIABLE) {
        var _variableName = argumentNode.value.name.value;
        throw new _error.GraphQLError("Argument \"".concat(name, "\" of required type \"").concat(String(argType), "\" ") + "was provided the variable \"$".concat(_variableName, "\" ") + 'which was not provided a runtime value.', [argumentNode.value]);
      } else {
        throw new _error.GraphQLError("Argument \"".concat(name, "\" of required type \"").concat(String(argType), "\" ") + 'was not provided.', [node]);
      }
    } else if (hasValue) {
