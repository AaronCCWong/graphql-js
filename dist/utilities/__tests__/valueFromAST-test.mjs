...
import { GraphQLEnumType, GraphQLInputObjectType, GraphQLList, GraphQLInt, GraphQLFloat, GraphQLString, GraphQLBoolean, GraphQLID, GraphQLNonNull } from '../../type';
...
describe('valueFromAST', function () {
  ...
  function testCaseWithVars(variables, type, valueText, expected) {
    expect(valueFromAST(parseValue(valueText), type, variables)).to.deep.equal(expected);
  }
  ...
  var nonNullBool = GraphQLNonNull(GraphQLBoolean); // [Boolean]

  var listOfBool = GraphQLList(GraphQLBoolean); // [Boolean!]

  var listOfNonNullBool = GraphQLList(nonNullBool); // [Boolean]!

  var nonNullListOfBool = GraphQLNonNull(listOfBool); // [Boolean!]!

  var nonNullListOfNonNullBool = GraphQLNonNull(listOfNonNullBool);
  ...
  var testInputObj = new GraphQLInputObjectType({
    name: 'TestInput',
    fields: {
      int: {
        type: GraphQLInt,
  ...
  it('accepts variable values assuming already coerced', function () {
    ...
    testCaseWithVars({
      var: true
    }, GraphQLBoolean, '$var', true);
    testCaseWithVars({
      var: null
    }, GraphQLBoolean, '$var', null);
  });
