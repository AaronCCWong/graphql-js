...
export function coerceValue(value, type, blameNode, path) {
  ...
  if (isNonNullType(type)) {
    if (isNullish(value)) {
      return ofErrors([coercionError("Expected non-nullable type ".concat(String(type), " not to be null"), blameNode, path)]);
    }
      ...
      if (hasOwnProperty.call(fields, fieldName)) {
        ...
        if (isInvalid(fieldValue)) {
          if (!isInvalid(field.defaultValue)) {
            ...
            _errors2 = add(_errors2, coercionError("Field ".concat(printPath(atPath(path, fieldName)), " of required ") + "type ".concat(String(field.type), " was not provided"), blameNode));
          }
        } else {
