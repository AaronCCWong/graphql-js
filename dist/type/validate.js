...
function validateSchema(schema) {
  ...
  !(0, _schema.isSchema)(schema) ? (0, _invariant.default)(0, "Expected ".concat(String(schema), " to be a GraphQL schema.")) : void 0; // If this Schema has already been validated, return the previous results.
...
function validateRootTypes(context) {
  ...
  if (!queryType) {
    ...
    context.reportError("Query root type must be Object type, it cannot be ".concat(String(queryType), "."), getOperationTypeNode(schema, queryType, 'query'));
  }

  var mutationType = schema.getMutationType();

  if (mutationType && !(0, _definition.isObjectType)(mutationType)) {
    context.reportError('Mutation root type must be Object type if provided, it cannot be ' + "".concat(String(mutationType), "."), getOperationTypeNode(schema, mutationType, 'mutation'));
  }

  var subscriptionType = schema.getSubscriptionType();

  if (subscriptionType && !(0, _definition.isObjectType)(subscriptionType)) {
    context.reportError('Subscription root type must be Object type if provided, it cannot be ' + "".concat(String(subscriptionType), "."), getOperationTypeNode(schema, subscriptionType, 'subscription'));
  }
}
...
function validateDirectives(context) {
  ...
  directives.forEach(function (directive) {
    ...
    if (!(0, _directives.isDirective)(directive)) {
      context.reportError("Expected directive but got: ".concat(String(directive), "."), directive && directive.astNode);
      return;
    ...
    directive.args.forEach(function (arg) {
      ...
      if (!(0, _definition.isInputType)(arg.type)) {
        context.reportError("The type of @".concat(directive.name, "(").concat(argName, ":) must be Input Type ") + "but got: ".concat(String(arg.type), "."), getDirectiveArgTypeNode(directive, argName));
      }
    });
  });
}
...
function validateTypes(context) {
  ...
  (0, _objectValues.default)(typeMap).forEach(function (type) {
    ...
    if (!(0, _definition.isNamedType)(type)) {
      context.reportError("Expected GraphQL named type but got: ".concat(String(type), "."), type && type.astNode);
      return;
...
function validateFields(context, type) {
  ...
  fields.forEach(function (field) {
    ...
    if (!(0, _definition.isOutputType)(field.type)) {
      context.reportError("The type of ".concat(type.name, ".").concat(field.name, " must be Output Type ") + "but got: ".concat(String(field.type), "."), getFieldTypeNode(type, field.name));
    } // Ensure the arguments are valid
    ...
    field.args.forEach(function (arg) {
      ...
      if (!(0, _definition.isInputType)(arg.type)) {
        context.reportError("The type of ".concat(type.name, ".").concat(field.name, "(").concat(argName, ":) must be Input ") + "Type but got: ".concat(String(arg.type), "."), getFieldArgTypeNode(type, field.name, argName));
      }
    });
  });
}
...
function validateObjectInterfaces(context, object) {
  ...
  object.getInterfaces().forEach(function (iface) {
    if (!(0, _definition.isInterfaceType)(iface)) {
      context.reportError("Type ".concat(String(object), " must only implement Interface types, ") + "it cannot implement ".concat(String(iface), "."), getImplementsInterfaceNode(object, iface));
      return;
    }
...
function validateObjectImplementsInterface(context, object, iface) {
  ...
  Object.keys(ifaceFieldMap).forEach(function (fieldName) {
    ...
    if (!(0, _typeComparators.isTypeSubTypeOf)(context.schema, objectField.type, ifaceField.type)) {
      context.reportError("Interface field ".concat(iface.name, ".").concat(fieldName, " expects type ") + "".concat(String(ifaceField.type), " but ").concat(object.name, ".").concat(fieldName, " ") + "is type ".concat(String(objectField.type), "."), [getFieldTypeNode(iface, fieldName), getFieldTypeNode(object, fieldName)]);
    ...
    ifaceField.args.forEach(function (ifaceArg) {
      ...
      if (!(0, _typeComparators.isEqualType)(ifaceArg.type, objectArg.type)) {
        context.reportError("Interface field argument ".concat(iface.name, ".").concat(fieldName, "(").concat(argName, ":) ") + "expects type ".concat(String(ifaceArg.type), " but ") + "".concat(object.name, ".").concat(fieldName, "(").concat(argName, ":) is type ") + "".concat(String(objectArg.type), "."), [getFieldArgTypeNode(iface, fieldName, argName), getFieldArgTypeNode(object, fieldName, argName)]);
    ...
    objectField.args.forEach(function (objectArg) {
      ...
      if (!ifaceArg && (0, _definition.isNonNullType)(objectArg.type)) {
        context.reportError("Object field argument ".concat(object.name, ".").concat(fieldName, "(").concat(argName, ":) ") + "is of required type ".concat(String(objectArg.type), " but is not also ") + "provided by the Interface field ".concat(iface.name, ".").concat(fieldName, "."), [getFieldArgTypeNode(object, fieldName, argName), getFieldNode(iface, fieldName)]);
      }
    });
  });
}
...
function validateUnionMembers(context, union) {
  ...
  memberTypes.forEach(function (memberType) {
    ...
    if (!(0, _definition.isObjectType)(memberType)) {
      context.reportError("Union type ".concat(union.name, " can only include Object types, ") + "it cannot include ".concat(String(memberType), "."), getUnionMemberTypeNodes(union, String(memberType)));
    }
  });
}
...
function validateInputFields(context, inputObj) {
  ...
  fields.forEach(function (field) {
    ...
    if (!(0, _definition.isInputType)(field.type)) {
      context.reportError("The type of ".concat(inputObj.name, ".").concat(field.name, " must be Input Type ") + "but got: ".concat(String(field.type), "."), field.astNode && field.astNode.type);
    }
  });
}
