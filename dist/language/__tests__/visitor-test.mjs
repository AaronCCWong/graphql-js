...
function checkVisitorFnArgs(ast, args, isEdited) {
  ...
  expect(_typeof(key)).to.be.oneOf(['number', 'string']);
...
describe('Visitor', function () {
  ...
  it('allows editing a node both on enter and on leave', function () {
    ...
    expect(editedAst).to.deep.equal(_objectSpread({}, ast, {
  ...
  it('Experimental: visits variables defined in fragments', function () {
    var ast = parse('fragment a($v: Boolean = false) on t { f }', {
      noLocation: true,
  ...
  describe('visitWithTypeInfo', function () {
    it('maintains type info during visit', function () {
      var visited = [];
      var typeInfo = new TypeInfo(testSchema);
      var ast = parse('{ human(id: 4) { name, pets { ... { name } }, unknown } }');
    ...
    it('maintains type info during edit', function () {
      var visited = [];
      var typeInfo = new TypeInfo(testSchema);
      var ast = parse('{ human(id: 4) { name, pets }, alien }');
      var editedAst = visit(ast, visitWithTypeInfo(typeInfo, {
        enter: function enter(node) {
          checkVisitorFnArgs(ast, arguments,
          /* isEdited */
          true);
