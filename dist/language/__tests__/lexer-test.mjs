...
function lexOne(str) {
  var lexer = createLexer(new Source(str));
...
describe('Lexer', function () {
  ...
  it('can be JSON.stringified or util.inspected', function () {
    var token = lexOne('foo');
    expect(JSON.stringify(token)).to.equal('{"kind":"Name","value":"foo","line":1,"column":1}'); // NB: util.inspect used to suck

    if (parseFloat(process.version.slice(1)) > 0.1) {
      expect(require('util').inspect(token)).to.equal("{ kind: 'Name', value: 'foo', line: 1, column: 1 }");
    }
  });
  ...
  it('updates line numbers in error for file context', function () {
    ...
    try {
      var str = '' + '\n' + '\n' + '     ?\n' + '\n';
      var source = new Source(str, 'foo.js', {
        line: 11,
        column: 12
      });
    ...
    expect(String(caughtError)).to.equal(dedent(_templateObject3));
  });
  it('updates column numbers in error for file context', function () {
    var caughtError;

    try {
      var source = new Source('?', 'foo.js', {
        line: 1,
        column: 5
      });
      createLexer(source).advance();
    ...
    expect(String(caughtError)).to.equal(dedent(_templateObject4));
  });
  ...
  it('lex reports useful information for dashes in names', function () {
    var q = 'a-b';
    var lexer = createLexer(new Source(q));
    ...
    expect(caughtError.message).to.equal('Syntax Error: Invalid number, expected digit but got: "b".');
  ...
  it('produces double linked list of tokens, including comments', function () {
    var lexer = createLexer(new Source("{\n      #comment\n      field\n    }"));
    ...
    expect(tokens.map(function (tok) {
      return tok.kind;
