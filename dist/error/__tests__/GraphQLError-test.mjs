...
describe('GraphQLError', function () {
  ...
  it('uses the stack of an original error', function () {
    ...
    var e = new GraphQLError('msg', undefined, undefined, undefined, undefined, original);
    expect(e.name).to.equal('GraphQLError');
  ...
  it('creates new stack if original error has no stack', function () {
    ...
    var e = new GraphQLError('msg', null, null, null, null, original);
    expect(e.name).to.equal('GraphQLError');
    expect(e.stack).to.be.a('string');
    expect(e.message).to.equal('msg');
    expect(e.originalError).to.equal(original);
  ...
  it('converts single node to positions and locations', function () {
    ...
    var ast = parse(source);
    var fieldNode = ast.definitions[0].selectionSet.selections[0];
    var e = new GraphQLError('msg', fieldNode); // Non-array value.
  ...
  it('converts node with loc.start === 0 to positions and locations', function () {
    ...
    var e = new GraphQLError('msg', [operationNode]);
  ...
  it('converts source and positions to locations', function () {
    var source = new Source("{\n      field\n    }");
    var e = new GraphQLError('msg', null, source, [10]);
  ...
  it('serializes to include message and locations', function () {
    var node = parse('{ field }').definitions[0].selectionSet.selections[0];
    var e = new GraphQLError('msg', [node]);
    expect(JSON.stringify(e)).to.equal('{"message":"msg","locations":[{"line":1,"column":3}]}');
  });
  ...
  it('default error formatter includes path', function () {
    var e = new GraphQLError('msg', null, null, null, ['path', 3, 'to', 'field']);
  ...
  it('default error formatter includes extension fields', function () {
    var e = new GraphQLError('msg', null, null, null, null, null, {
      foo: 'bar'
    });
    expect(formatError(e)).to.deep.equal({
      message: 'msg',
